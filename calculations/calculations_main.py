...
#    transactions_subset['AGGREGATED_VOLUME'] = transactions_subset.groupby('ASSET_ID')['EFFECTIVE_VOLUME'].cumsum()
    ...
    id_col = 'PORTFOLIO_ID' if 'PORTFOLIO_ID' in daily_data.columns else 'ACCOUNT_OWNER_ID'
    daily_data = daily_data[['TIMESTAMP', 'CONVERTED_PRICE', 'AGGREGATED_VOLUME', id_col]].ffill().fillna(0)
    ...
    daily_data[id_col] = daily_data[id_col].astype(int)

    output_col = 'PORTFOLIO_ID' if id_col == 'PORTFOLIO_ID' else 'ACCOUNT_OWNER_ID'
    if output_col != id_col:
        daily_data[output_col] = daily_data[id_col]

    result = daily_data[['TIMESTAMP','ASSET_ID',output_col,'AGGREGATED_VALUE']]
