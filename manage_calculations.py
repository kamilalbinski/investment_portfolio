...
def calculate_current_values(portfolio_id=None, return_totals=False):
    ...
    market_df = get_current_portfolio_data(table_name='CURRENT_HOLDINGS_ALL', portfolio_id=portfolio_id)
...
def calculate_all_portfolios_over_time(portfolio_ids):
    ...
    return pd.concat(all_portfolios, keys=portfolio_ids).reset_index(drop=True)
